...
def process_o_line_query() -> pd.DataFrame:
    o_line = di.Dataset()
    o_line.set_query("select efficiency, player_display_name, (cast(rush_yards_over_expected as decimal)/rush_attempts) as rush_over_exp_pg, team_abbr from nfl_data.ngs_rushing where season = 2024 and week = 0 order by rush_over_exp_pg desc")
...
def process_rb_roe_query() -> pd.DataFrame:
    rb_roe = di.Dataset()
    rb_roe.set_query("select SUM(rushing_yards_before_contact_avg) as szn_rush_yds_before_contact_avg_sum, MAX(team) as team, sum(carries) as szn_carries from nfl_data.ADVSTATS_WEEK_RUSH where season = 2024 and week <= 17 group by team order by SUM(rushing_yards_before_contact_avg) desc")
